...
def preparar_entorno():
    """Asegura que yt-dlp esté al día."""
    try:
        import yt_dlp
...
def gestionar_archivos():
    archivos = {
        'que_busco.txt': '# Pon aquí links de videos, canales o nombres de canciones\n',
        'black_list.txt': '# IDs o nombres de canales a ignorar\n'
    }
...
def auditar_y_extraer_links(info):
    ...
    # Detección de Scams en descripción
    if any(x in desc.lower() for x in ["content id", "claim", "reclamado", "copyright", "distrokid"]):
        score += 50
    ...
    for c in comments[:40]:
        t = c.get('text', '').lower()
        if any(w in t for w in ["scam", "fake", "strike", "reclamo", "estafa", "copyrighted"]):
            score += 25
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        for item in busquedas:
            ...
            try:
                ...
                print(f"❌ Error o duplicado: {item}")
